    ...
    */
    return function(config) {
        ...
        if (!number){
            ...
            me.snakeHead.elm.className += " snake-snakebody-alive2";
        }
        else if (number === 2){
            me.snakeHead.elm.className += " snake-snakebody-alive3";
        }
        else if (number === 3){
            me.snakeHead.elm.className += " snake-snakebody-alive4";
        }
        ...
        me.eatFood = function() {
            ...
            while (ii--) {
                ...
                if (number === 0){
                    ...
                    me.snakeBody[index].elm.className += " snake-snakebody-alive2";
                }
                else if (number === 2){
                    me.snakeBody[index].elm.className += " snake-snakebody-alive3";
                }
                else if (number === 3){
                    me.snakeBody[index].elm.className += " snake-snakebody-alive4";
                }
        ...
        me.reset = function() {
            ...
            for (var ii = 0; ii < blocks.length; ii++) {
                ...
                if (!number){
                    ...
                    blocks[ii].elm.className += " snake-snakebody-alive2";
                }                
                else if (number === 2){
                    blocks[ii].elm.className += " snake-snakebody-alive3";
                }
                else if (number === 3){
                    blocks[ii].elm.className += " snake-snakebody-alive4";
                }
            }
    ...
    */
    ...
    return function(inputConfig) {
        ...
        var me = this,
            ...
            // Board components
            lengthPanels = [],
            elmContainer, elmPlayingField, elmAboutPanel, elmLengthPanel, elmWelcome, elmTryAgain, elmPauseScreen,tmpElm;
        // --- public variables ---
        me.grid = [];
        ...
        function createBoardElements() {
            ...
            elmAboutPanel.innerHTML = "<a href='http://patorjk.com/blog/software/' class='snake-link'>Windask's fork of snake from patorjk.com</a> - <a href='https://github.com/patorjk/JavaScript-Snake' class='snake-link'>source code</a>";
            
            for (var i = 0; i < players; i++){
                elmLengthPanel = document.createElement("div");
                elmLengthPanel.className = "snake-panel-component";
                elmLengthPanel.innerHTML = "Snake " + i + " - Length: 1";
                lengthPanels.push(elmLengthPanel);
            }
            ...
            for (var i = 0; i < players; i++){
                elmContainer.appendChild(lengthPanels[i]);
            }
            elmContainer.appendChild(elmWelcome);
            elmContainer.appendChild(elmTryAgain);
            
            for (var i = 0; i < players; i++){
                snake.push(new SNAKE.Snake({playingBoard:me,startRow:2+i,startCol:2+i, number:i}));
            }
        ...
        me.loadGame = function() {
                SNAKE.removeEventListener(window, "keyup", me.kbShortcut, false);
        ...
        function createWelcomeElement() {
            ...
            SNAKE.addEventListener(window, "keyup", me.kbShortcut, false);
            SNAKE.addEventListener(welcomeStart, "click", me.loadGame, false);
        ...
        function createTryAgainElement() {
            ...
            var max = 0;
            var snakeNum = "";
            if (snake.length === players){
                for (var i = 0; i < players; i++){
                    if (snake[i].snakeLength > max){
                        max = snake[i].snakeLength;
                        snakeNum = i.toString();
                    }
                    else if (snake[i].snakeLength == max){
                        snakeNum += ", " + i;
                    }
                }
            }
            tryAgainTxt.innerHTML = "JavaScript Snake<p></p>Snakes " + snakeNum + " win(s) with: " + max + " points.<p></p>";
        ...
        me.setPausedClient = function(val) {
            if (boardState === 2){
                isPaused = val;
                for (var i = 0; i < players; i++){
                    snake[i].setPaused(val);
        ...
        */   
        me.resetBoard = function() {
            ...
            for (var i = 0; i < players; i++){
                lengthPanels[i].innerHTML = "Snake " + i + " - Length: 1";
            }
        ...
        */ 
        me.setupPlayingField = function () {
            ...
            for (var i = 0; i < players; i++){
                lengthPanels[i].style.top = pLabelTop;
                lengthPanels[i].style.left = cWidth -180*(i + 1) + "px";
            }
            ...
            myKeyListener = function(evt) {
                ...
                if (me.getBoardState() === 1) {
                    ...
                    myKeyListener = function(evt) {
                        ...
                        for (var i = 0; i < players; i++){
                            snake[i].handleArrowKeys(keyNum);
                        }
        ...
        * @method foodEaten
        */ 
        me.foodEaten = function(number) {
            lengthPanels[number].innerHTML = "Snake " + number + " - Length: " + snake[number].snakeLength;
            myFood.randomlyPlaceFood();
        };
        ...
        */ 
        me.handleDeath = function() {
            numAlive--;
            if (numAlive <= 0){
                var index = Math.max(getNextHighestZIndex( snake[0].snakeBody), getNextHighestZIndex( {tmp:{elm:myFood.getFoodElement()}} ));
